...
> __NOTE:__ _Il s’agit ici des instructions manuel à suivre. Cependant, à terme,
  il est prévu qu'un installer soit disponible pour simplifier le démarage._
...
Afin de pouvoir utiliser facilement les commandes fournies par npm, installées
dans votre répertoire projet, il est recommandé de rajouter
`./node_modules/.bin` à votre `PATH`.
...
* `/src/assets`: L’ensemble des fichiers qui doivent être utilisés par le projet tel quel.
...
**assets** : Déplace et optimise les assets statiques du projet
...
un nouveau projet d'intégration. Ils garantissent un workflow de travail optimal.
...
Ils sont tous utilisables tel quel, mais le starter kit est suffisamment flexible pour s’adapter à vos besoins. Le choix d'utilisation de ces outils se fait via le fichier `.gsk/config.json`, voir les [instructions de configuration](.gsk/readme.md).
