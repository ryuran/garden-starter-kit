...
var PLUGIN_NAME = 'gulp-drupal-twig';
...
function gulpDrupalTwig(templateSrc, options) {
  ...
  var templateAttribute = options.templateAttribute || '#theme';
  templateAttribute = templateAttribute.split('.');
  var dataParser = options.dataParser || function (data) { return data };
  ...
  return through.obj(function (file, enc, cb) {
    if (file.isNull()) {
      ...
      Promise.resolve(file)
        .then(processFile)
        .done(function(file) { cb(null, file); }, function(err) { cb(err, null); });
...
gulpDrupalTwig.twig = Twig;

// Exporting the plugin main function
module.exports = gulpDrupalTwig;
